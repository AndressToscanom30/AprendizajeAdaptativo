...
export const enviarRespuestas = async (req, res) => {
    ...
    try {
        ...
        for (const r of respuestas) {
            ...
            const pregunta = await Pregunta.findByPk(preguntaId, { include: [{ model: OpcionPregunta, as: "Opcion" }], transaction: t });
            ...
            const via = await PreguntaEvaluacion.findOne({ where: { evaluacionId: intento.evaluacionId, preguntaId }, transaction: t });
            ...
            if (["opcion_multiple", "verdadero_falso"].includes(pregunta.tipo)) {
                ...
                const selected = (opcionSeleccionadaId || []).sort();
                ...
                isCorrect = JSON.stringify(correctPairs) === JSON.stringify(matchPairs || []);
                pointsAwarded = isCorrect ? pointsForQuestion : 0;
            } else {
                es_correcta = null;
...
export const obtenerResultadoIntento = async (req, res) => {
    try {
        const { intetoId } = req.params;
        const intento = await Intento.findByPk(intetoId, {
            include: [{ model: IntentoRespuesta, as: "Respuesta" }]
        });
